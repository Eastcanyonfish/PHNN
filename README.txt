...
- TensorFlow 2.10.0: A deep learning framework used for the development and testing of PHNN models. TensorFlow provides a rich set of APIs and tools that make the construction, training, and evaluation of models more efficient.
 If you want to use the code in a old environment, you can simply activate the following code:
from PHNN1 import PHNN_Model

In order to facilitate users in running the code in updated environments, we have prepared a version tailored for the latest development environment, which requests the modifications in model configuration. The code has been tested and executed successfully on both Windows and macOS platforms using Python 3.10 and TensorFlow 2.17. If you wish to utilize the code in a similar environment, you can simply activate the following code:
